...
function abos_compter_dist() {
	...
	if (!$last) {
		$last = sql_getfetsel('date', 'spip_abonnements', sql_in('statut', ['ok', 'resilie']), '', 'date', '0,1');
		if (
			!$last
			or !intval($last)
			or !strtotime($last)
...
function abos_compter_jour($day) {
	...
	$where_abos[] = sql_in('mode_paiement', ['gratuit', 'parrain'], 'NOT');
	...
	$exclus = sql_allfetsel('DISTINCT id_auteur', 'spip_abonnements', 'date<' . sql_quote($day_start) . ' AND ' . sql_in('statut', ['ok', 'resilie']) . ' AND ' . sql_in('id_auteur', $id_auteurs) . ' AND ' . sql_in('mode_paiement', ['gratuit', 'parrain'], 'NOT'));
	...
	// ne pas compter les abonnements gratuit ou parrain
	$where_abos[] = sql_in('mode_paiement', ['gratuit', 'parrain'], 'NOT');
	$where_abos[] = sql_in('statut', ['ok', 'resilie']);
	/*
...
function abos_reporting_decompte($nb_mois = 6) {
	...
	$texte .= '<h2>' . _T('abonnement:derniers_jours_nb', ['nb' => 7]) . "</h2>
<table class='spip'>
$head
$lignes
</table>";
	...
	$texte .= '<h2>' . _T('abonnement:dernieres_semaines_nb', ['nb' => 4]) . "</h2>
<table class='spip'>
$head
$lignes
</table>";
	...
	$texte .= '<h2>' . _T('abonnement:derniers_mois_nb', ['nb' => $nb_mois]) . "</h2>
<table class='spip'>
$head
$lignes
</table>";
...
function abos_sum_lines($rows) {
	$total = [
		...
		'ventil_abonnements' => [],
		'ventil_abonnements_new' => [],
		'ventil_abonnements_plus' => [],
		'ventil_abonnements_moins' => [],
	];
	$first = reset($rows);
	...
	foreach ($rows as $row) {
		foreach (['abonnements_new', 'abonnements_plus', 'abonnements_moins'] as $quoi) {
...
function abos_reporting_parrainages($nb_mois = 6) {
	...
	for ($i = 0; $i < $nb_mois; $i++) {
		...
		foreach ($parrainages as $p) {
			$nba = sql_countsel('spip_abonnements', 'id_auteur=' . intval($p['id_auteur']) . ' AND date>' . sql_quote($p['date'] . ' AND ' . sql_in('statut', ['ok', 'resilie'])));
			if ($nba > 0) {
				$nb_convertis++;
	...
	$texte .= '<h2>' . _T('abonnement:derniers_mois_nb', ['nb' => 6]) . "</h2>
<table class='spip'>
$head
$lignes
</table>";
