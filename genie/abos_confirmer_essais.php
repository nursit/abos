...
if (!defined('_DUREE_CONSULTATION_ESSAI')) define('_DUREE_CONSULTATION_ESSAI',15*60); // 15 minutes
...
function genie_abos_confirmer_essais_dist(){
	...
	$res = sql_select('A.*,T.pay_id','spip_abonnements AS A JOIN spip_transactions AS T ON T.id_transaction=A.abonne_uid',
	...
	while ($row = sql_fetch($res)){
		...
		if (strncmp($r['statut'],'echec',5)==0){
			...
			if ($r['date_transaction']<$old){
				...
				sql_updateq("spip_abonnements",array('statut'=>'resilie'),'id_abonnement='.intval($row['id_abonnement']));
				continue;
			}
