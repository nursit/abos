...
if (!defined('_ABOS_RELANCE_POOL')) {
	define('_ABOS_RELANCE_POOL', 20);
}
...
function genie_abos_relancer($t) {
	...
	sql_updateq('spip_abonnements', ['relance' => $premiere_relance], 'statut=' . sql_quote('ok') . ' AND relance=' . sql_quote('') . ' AND date_fin>date_debut AND date_fin<=' . sql_quote($date_fin));
	...
	if (count($rappels)) {
		...
		while ($nb--) {
			if ($row = sql_fetsel('id_abonnement,date_fin,relance', 'spip_abonnements', $where, '', 'date_fin', '0,1')) {
				...
				sql_updateq('spip_abonnements', ['relance' => abos_prochaine_relance($row['date_fin'], $now)], 'id_abonnement=' . intval($row['id_abonnement']));
			} else {
				$nb = 0;
			}
		}
